...
struct F { //std::vector<T> &v;
	...
	public:
	T d;
	typedef std::initializer_list<int> init_int;
	F()  = default;	
...
std::thread **create_thread(int size) noexcept  { //undefined behaviour
	std::vector<int>  v;
	int d;
	auto b = std::bind(f, v);
	...
	for(int i=0;i<size+2; i++) {
		...
		t[i+2] = new std::thread{std::bind(compute_high, size,0)};
		(*t[i+2]).join();
	}
	return t;
}
